...
export default function AdminDashboard({ onLogout, additionalMembers }: AdminDashboardProps) {
  ...
  const handleChangeMemberStatus = async (id: string, targetStatus: string) => {
    let payload: any = {};
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                ...
                className="bg-gold hover:bg-gold-light text-[#0A0D14] px-6 py-3.5 rounded-xl text-xs font-black tracking-widest uppercase transition-colors shrink-0 cursor-pointer font-bold"
              >
                Rechercher statut
              </button>
            </div>
        ...
        <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              className="w-full bg-slate-950 border border-white/5 rounded-xl pl-11 pr-4 py-3.5 text-xs text-white outline-none focus:border-[#D4AF37]/50 transition-colors"
              value={searchQuery}
                ...
                <AnimatePresence>
                  {filteredMembers.map((member) => (
                    <motion.div 
                      ...
                      className="p-6 rounded-2xl bg-slate-900 border border-white/5 flex flex-col justify-between hover:border-[#D4AF37]/30 transition-all group animate-[fadeIn_0.3s_ease-out]"
                    >
                      <div>
